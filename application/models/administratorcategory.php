<?php 
class AdministratorCategory extends CI_Model {
	...
	public function categoryDetails(){
		...
		$aColumns = array('id','name','teacher','added_date','1' );
		...
		foreach ($rResult->result_array() as $aRow){
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ ){
				if ( $aColumns[$i] == 'id' ){
                    ...
                    $category_id = $aRow['id'];
                    $category_name = $aRow['name'];
                    $teacher_name = $aRow['teacher'];
					$row[] = '<a class="small black button" href="javascript:;" id="test" style="text-decoration:none;"
					onclick="update_category(\''.$category_id.'\',\''.$category_name.'\',\''.$teacher_name.'\');">Edit</a>|<a class="small black button" href="javascript:;" style="text-decoration:none;" onclick="delete_category('.$category_id.');">Delete</a>';
