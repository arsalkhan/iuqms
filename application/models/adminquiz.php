<?php
class AdminQuiz extends CI_Model {
		...
		foreach ($rResult->result_array() as $aRow){
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ ){
				if ( $aColumns[$i] == 'id' ){
                    ...
                    $quiz_id = $aRow['id'];
                    $quiz_title = $aRow['title'];
                    $quiz_description = $aRow['description'];
					$row[] = '<a class="small black button" href="javascript:;" id="test" style="text-decoration:none;"
					onclick="update_quiz(\''.$quiz_id.'\',\''.$quiz_title.'\',\''.$quiz_description.'\',\''.$cat_name.'\');">Edit</a>|<a class="small black button" href="javascript:;" style="text-decoration:none;" onclick="delete_quiz('.$quiz_id.');">Delete</a>';
				}
				else if ( $aColumns[$i] != ' ' ){
					/* General output */
					$row[] = $aRow[ $aColumns[$i] ];
	...
	//for edit user info.
	public function editProductInfo($data,$user_id){
